# Data Structures

The Data Structures package contains common data structures and design pattern implementations for Unity projects. It also contains dozens of class extensions for common collection types and interfaces.

### Structs

- [Bool3](xref:Zigurous.DataStructures.Bool3)
- [Bitmask](xref:Zigurous.DataStructures.Bitmask)
- [Quantity](xref:Zigurous.DataStructures.Quantity`1)
- [Size](xref:Zigurous.DataStructures.Size)
- [GridSize](xref:Zigurous.DataStructures.GridSize)
- [Range](xref:Zigurous.DataStructures.Range`1)
- [ClampedRange](xref:Zigurous.DataStructures.ClampedRange)
- [ColorRange](xref:Zigurous.DataStructures.ColorRange)
- [EulerRange](xref:Zigurous.DataStructures.EulerRange)
- [FloatRange](xref:Zigurous.DataStructures.FloatRange)
- [IntRange](xref:Zigurous.DataStructures.IntRange)
- [UIntRange](xref:Zigurous.DataStructures.UIntRange)
- [UnitIntervalRange](xref:Zigurous.DataStructures.UnitIntervalRange)
- [Vector2Range](xref:Zigurous.DataStructures.Vector2Range)
- [Vector3Range](xref:Zigurous.DataStructures.Vector3Range)
- [Vector4Range](xref:Zigurous.DataStructures.Vector4Range)

### Patterns

- [SingletonBehavior](xref:Zigurous.DataStructures.SingletonBehavior`1)
- [ValueAccumulator](xref:Zigurous.DataStructures.ValueAccumulator`1)
- [ObjectPool](xref:Zigurous.DataStructures.ObjectPool`1)
- [Modules](xref:Zigurous.DataStructures.Modules`1)

### Extensions

- [ArrayExtensions](xref:Zigurous.DataStructures.ArrayExtensions)
- [ComparableExtensions](xref:Zigurous.DataStructures.ComparableExtensions)
- [DictionaryExtensions](xref:Zigurous.DataStructures.DictionaryExtensions)
- [HashSetExtensions](xref:Zigurous.DataStructures.HashSetExtensions)
- [ListExtensions](xref:Zigurous.DataStructures.ListExtensions)
- [SortedSetExtensions](xref:Zigurous.DataStructures.SortedSetExtensions)
- [StringExtensions](xref:Zigurous.DataStructures.StringExtensions)

### Utilities

- [HashCode](xref:Zigurous.DataStructures.HashCode)
- [Identifier](xref:Zigurous.DataStructures.Identifier)
